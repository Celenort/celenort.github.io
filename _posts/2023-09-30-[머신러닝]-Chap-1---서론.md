...
${leadingWhitespace}$$
${leadingWhitespace}\text{let Hypothesis space } \chi \text{ must be discrete} \\ P({\bf x}) : \text{prob. of samp } {\bf x} \\  \text{II}(\cdot) : \text{1 if } \cdot \text{ is truth else 0} \\ P(h \vert X, A_a) : \text{X로 학습한 Algorithm a가 생성한 h의 확률} \\ f : \text{objective function}
${leadingWhitespace}$$
...
${leadingWhitespace}{% raw %}
${leadingWhitespace}$$
${leadingWhitespace}E_{ote}(A_a \vert X, f) = \sum_h \sum_{x\in \chi-X} P({\bf x}) \cdot \text{II}(h({\bf x})-f({\bf x})) P(h \vert X, A_a)\\ = \sum_{x\in \chi-X} P({\bf x}) \sum_h P(h \vert X, A_a) \sum_f \text{II}(h({\bf x})-f({\bf x})) \\  = \sum_{x\in \chi-X} P({\bf x}) \sum_h P(h \vert X, A_a) \cdot \frac{1}{2} \cdot 2^{\vert \chi \vert} \\ = 2^{\vert \chi \vert - 1} \sum_{x\in \chi-X} P({\bf x})
${leadingWhitespace}$$
${leadingWhitespace}{% endraw %}
