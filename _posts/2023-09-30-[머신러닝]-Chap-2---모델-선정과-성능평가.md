...
- $\text{#}$ of data samples가 $m$일 때, $k=m$인 cross validation을 leave-one-out cross validaion이라 함.
...
- 수학적으로 ,
	...
	\lim\limits_{m\rightarrow \infty} \bigg(1-\frac{1}{m}\bigg)^m = \frac{1}{e}\approx 0.368{% raw %}
	\lim\limits_{m\rightarrow \infty} \bigg(1-\frac{1}{m}\bigg)^m = \frac{1}{e}\approx 0.368$$
	\lim\limits_{m\rightarrow \infty} \bigg(1-\frac{1}{m}\bigg)^m = \frac{1}{e}\approx 0.3681375
	\lim\limits_{m\rightarrow \infty} \bigg(1-\frac{1}{m}\bigg)^m = \frac{1}{e}\approx 0.368$$
	\lim\limits_{m\rightarrow \infty} \bigg(1-\frac{1}{m}\bigg)^m = \frac{1}{e}\approx 0.368{% endraw %}
...
E(f;D) = \frac{1}{m} \sum_{i=1}^m (f({\bf x_i})-y_i)^2{% raw %}
E(f;D) = \frac{1}{m} \sum_{i=1}^m (f({\bf x_i})-y_i)^2$$
E(f;D) = \frac{1}{m} \sum_{i=1}^m (f({\bf x_i})-y_i)^21747
E(f;D) = \frac{1}{m} \sum_{i=1}^m (f({\bf x_i})-y_i)^2$$
E(f;D) = \frac{1}{m} \sum_{i=1}^m (f({\bf x_i})-y_i)^2{% endraw %}


확률 밀도 함수로 표현하면, 


E(f;D) = \int_{x\sim D} (f({\bf x})-y)^2 p({\bf x} ) d({\bf x}){% raw %}
E(f;D) = \int_{x\sim D} (f({\bf x})-y)^2 p({\bf x} ) d({\bf x})$$
E(f;D) = \int_{x\sim D} (f({\bf x})-y)^2 p({\bf x} ) d({\bf x})1829
E(f;D) = \int_{x\sim D} (f({\bf x})-y)^2 p({\bf x} ) d({\bf x})$$
E(f;D) = \int_{x\sim D} (f({\bf x})-y)^2 p({\bf x} ) d({\bf x}){% endraw %}
...
- 오차율, 정확도
	...
	acc(f;D) = \frac{1}{m} \sum_{i=1}^m II(f({\bf x_i})= y_i){% raw %}
	acc(f;D) = \frac{1}{m} \sum_{i=1}^m II(f({\bf x_i})= y_i)$$
	acc(f;D) = \frac{1}{m} \sum_{i=1}^m II(f({\bf x_i})= y_i)1997
	acc(f;D) = \frac{1}{m} \sum_{i=1}^m II(f({\bf x_i})= y_i)$$
	acc(f;D) = \frac{1}{m} \sum_{i=1}^m II(f({\bf x_i})= y_i){% endraw %}
...
undefined- 정확도(accuracy) in confusion matrix
	...
	acc = \frac{TP+TN} {TP+TN+FP+FN}{% raw %}
	acc = \frac{TP+TN} {TP+TN+FP+FN}$$
	acc = \frac{TP+TN} {TP+TN+FP+FN}3080
	acc = \frac{TP+TN} {TP+TN+FP+FN}$$
	acc = \frac{TP+TN} {TP+TN+FP+FN}{% endraw %}
...
- 정밀도. positive라고 예측한 것 들 중 실제로 positive인 것들의 빈도
	...
	prec = \frac{TP} {TP+FP}{% raw %}
	prec = \frac{TP} {TP+FP}$$
	prec = \frac{TP} {TP+FP}3298
	prec = \frac{TP} {TP+FP}$$
	prec = \frac{TP} {TP+FP}{% endraw %}
...
- 재현율. ground truth가 positive인 것들 중 모델이 positive라고 예측한 것의 빈도
	...
	rec = \frac{TP} {TP + FN}{% raw %}
	rec = \frac{TP} {TP + FN}$$
	rec = \frac{TP} {TP + FN}3408
	rec = \frac{TP} {TP + FN}$$
	rec = \frac{TP} {TP + FN}{% endraw %}
...
: y=x와 PR curve가 만나는 점, PR curve 가 intersect할 경우 평가하는 방법중 하나.
...
- 정밀도(P), 재현율(R)라 할 때
	...
	\text{F1 score = } \frac{2PR}{P+R}{% raw %}
	\text{F1 score = } \frac{2PR}{P+R}$$
	\text{F1 score = } \frac{2PR}{P+R}4486
	\text{F1 score = } \frac{2PR}{P+R}$$
	\text{F1 score = } \frac{2PR}{P+R}{% endraw %}


### F-beta-score

- F1-score는 Precision, Recall의 가중치를 동일하게 둔 경우이며, 실제 상황에서 어느 특정 값에 가중치를 두어야 할 경우 F-beta score를 사용함. $\beta>1$인 경우는 R의 영향이 더 크며, $\beta<1$의 경우는 P의 영향이 더 큼.

	F_\beta = \frac{(1+\beta^2)\times P \times R} {(\beta^2\times P) + R}{% raw %}
	F_\beta = \frac{(1+\beta^2)\times P \times R} {(\beta^2\times P) + R}$$
	F_\beta = \frac{(1+\beta^2)\times P \times R} {(\beta^2\times P) + R}4705
	F_\beta = \frac{(1+\beta^2)\times P \times R} {(\beta^2\times P) + R}$$
	F_\beta = \frac{(1+\beta^2)\times P \times R} {(\beta^2\times P) + R}{% endraw %}
...
- threshold를 정하기 위한 도구 : ROC
	...
	TPR = \frac{TP}{TP+FN}{% raw %}
	TPR = \frac{TP}{TP+FN}$$
	TPR = \frac{TP}{TP+FN}5093
	TPR = \frac{TP}{TP+FN}$$
	TPR = \frac{TP}{TP+FN}{% endraw %}


	FPR = \frac{FP}{TN+FP}{% raw %}
	FPR = \frac{FP}{TN+FP}$$
	FPR = \frac{FP}{TN+FP}5127
	FPR = \frac{FP}{TN+FP}$$
	FPR = \frac{FP}{TN+FP}{% endraw %}
...
- x axis of cost curves is probability cost of positive class
	...
	P(+)cost = \frac{p \times cost_{01}}{p\times cost_{01}+(1-p)\times cost_{10}}{% raw %}
	P(+)cost = \frac{p \times cost_{01}}{p\times cost_{01}+(1-p)\times cost_{10}}$$
	P(+)cost = \frac{p \times cost_{01}}{p\times cost_{01}+(1-p)\times cost_{10}}6204
	P(+)cost = \frac{p \times cost_{01}}{p\times cost_{01}+(1-p)\times cost_{10}}$$
	P(+)cost = \frac{p \times cost_{01}}{p\times cost_{01}+(1-p)\times cost_{10}}{% endraw %}
...
- y axis is normalized cost which takes values from [0,1]
	...
	cost_{norm} = \frac{FNR \times p \times cost_{01} + FPR \times (1-p)\times cost_{10}} {p\times cost_{01} + (1-p)\times cost_{10}}{% raw %}
	cost_{norm} = \frac{FNR \times p \times cost_{01} + FPR \times (1-p)\times cost_{10}} {p\times cost_{01} + (1-p)\times cost_{10}}$$
	cost_{norm} = \frac{FNR \times p \times cost_{01} + FPR \times (1-p)\times cost_{10}} {p\times cost_{01} + (1-p)\times cost_{10}}6479
	cost_{norm} = \frac{FNR \times p \times cost_{01} + FPR \times (1-p)\times cost_{10}} {p\times cost_{01} + (1-p)\times cost_{10}}$$
	cost_{norm} = \frac{FNR \times p \times cost_{01} + FPR \times (1-p)\times cost_{10}} {p\times cost_{01} + (1-p)\times cost_{10}}{% endraw %}
	...
	- $FNR = 1-TPR$
- p=0일 때의 $cost_{norm}=FNR$, p=1일 때의 $cost_{norm} = FPR$이므로, cost-plane 상에 각각의 ROC curve 의 점들에 대한 (0,FPR), (1,1-TPR)을 잇는 직선들을 모두 plot 하여 직선들 아래의 면적이 기대 총 비용 (expected total cost)가 될 것.
...
- 이 때 각각의 테스트 오차율이 독립표본이라면 검정통계량은 다음과 같음.
	...
	\tau_t = \frac{\sqrt k (\mu-\epsilon_0)}{\sigma}\sim t(k-1){% raw %}
	\tau_t = \frac{\sqrt k (\mu-\epsilon_0)}{\sigma}\sim t(k-1)$$
	\tau_t = \frac{\sqrt k (\mu-\epsilon_0)}{\sigma}\sim t(k-1)7834
	\tau_t = \frac{\sqrt k (\mu-\epsilon_0)}{\sigma}\sim t(k-1)$$
	\tau_t = \frac{\sqrt k (\mu-\epsilon_0)}{\sigma}\sim t(k-1){% endraw %}
...
- 두 학습기의 오차율이 같다는 귀무가설을 세우고, 동일한 test-set을 사용하여 나온 두 학습기의 오차율의 차($\Delta_i$)를 t-test 한다.
	...
	\tau_t = \vert\frac{\sqrt{k}\mu}{\sigma}\vert{% raw %}
	\tau_t = \vert\frac{\sqrt{k}\mu}{\sigma}\vert$$
	\tau_t = \vert\frac{\sqrt{k}\mu}{\sigma}\vert8137
	\tau_t = \vert\frac{\sqrt{k}\mu}{\sigma}\vert$$
	\tau_t = \vert\frac{\sqrt{k}\mu}{\sigma}\vert{% endraw %}
...
- testing set을 5개로 나누고, 각각의 set을 또 2개로 나누어 2-fold cross-validation을 수행한다. 2개의 오차율 차이값이 도출$\Delta_i^1, \Delta_i^2 \text{ for i=1~5}$)
- 비독립성의 완화를 위해(?) i=1일때의 오차율의 평균을 $\mu$로 활용
- 분산은 각각의 5개에 대한 분산의 합을 이용.

	\sigma_i^2 = \bigg(\Delta_i^1-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2+\bigg(\Delta_i^2-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2{% raw %}
	\sigma_i^2 = \bigg(\Delta_i^1-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2+\bigg(\Delta_i^2-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2$$
	\sigma_i^2 = \bigg(\Delta_i^1-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2+\bigg(\Delta_i^2-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^28533
	\sigma_i^2 = \bigg(\Delta_i^1-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2+\bigg(\Delta_i^2-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2$$
	\sigma_i^2 = \bigg(\Delta_i^1-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2+\bigg(\Delta_i^2-\frac{\Delta_i^1+\Delta_i^2}{2}\bigg)^2{% endraw %}

- 이렇게 계산된 T검정통계량은

	\tau_t = \frac{\mu}{\sqrt{0.2 \sum_{i=1}^5 \sigma_i^2}}{% raw %}
	\tau_t = \frac{\mu}{\sqrt{0.2 \sum_{i=1}^5 \sigma_i^2}}$$
	\tau_t = \frac{\mu}{\sqrt{0.2 \sum_{i=1}^5 \sigma_i^2}}8689
	\tau_t = \frac{\mu}{\sqrt{0.2 \sum_{i=1}^5 \sigma_i^2}}$$
	\tau_t = \frac{\mu}{\sqrt{0.2 \sum_{i=1}^5 \sigma_i^2}}{% endraw %}
...
undefined- 두 학습기의 성능이 동일하다는 것은 $e_{01}=e_{10}$임. 즉 서로 다르게 판별한 개수의 차가 정규분포를 이룰 것이므로,
	...
	\tau_{\chi^2} = \frac{(\vert e_{01}-e_{10}\vert -1)^2}{e_{01}+e_{10}} \sim \chi^2_1{% raw %}
	\tau_{\chi^2} = \frac{(\vert e_{01}-e_{10}\vert -1)^2}{e_{01}+e_{10}} \sim \chi^2_1$$
	\tau_{\chi^2} = \frac{(\vert e_{01}-e_{10}\vert -1)^2}{e_{01}+e_{10}} \sim \chi^2_19066
	\tau_{\chi^2} = \frac{(\vert e_{01}-e_{10}\vert -1)^2}{e_{01}+e_{10}} \sim \chi^2_1$$
	\tau_{\chi^2} = \frac{(\vert e_{01}-e_{10}\vert -1)^2}{e_{01}+e_{10}} \sim \chi^2_1{% endraw %}
...
- 각각의 알고리즘의 평균 등수에 대한 평균과 분산을 구할 수 있으며, N개의 data set, k개의 algorithm에 대해
	...
	\tau_{\chi^2} = \frac{k-1}{k} \cdot \frac{12N}{k^2-1} \sum_{i=1}^k \bigg(r_i-\frac{k+1}{2}\bigg)^2 \sim \chi^2_{k-1}{% raw %}
	\tau_{\chi^2} = \frac{k-1}{k} \cdot \frac{12N}{k^2-1} \sum_{i=1}^k \bigg(r_i-\frac{k+1}{2}\bigg)^2 \sim \chi^2_{k-1}$$
	\tau_{\chi^2} = \frac{k-1}{k} \cdot \frac{12N}{k^2-1} \sum_{i=1}^k \bigg(r_i-\frac{k+1}{2}\bigg)^2 \sim \chi^2_{k-1}9418
	\tau_{\chi^2} = \frac{k-1}{k} \cdot \frac{12N}{k^2-1} \sum_{i=1}^k \bigg(r_i-\frac{k+1}{2}\bigg)^2 \sim \chi^2_{k-1}$$
	\tau_{\chi^2} = \frac{k-1}{k} \cdot \frac{12N}{k^2-1} \sum_{i=1}^k \bigg(r_i-\frac{k+1}{2}\bigg)^2 \sim \chi^2_{k-1}{% endraw %}

- 위 검정 통계량은 자유도가 k-1인 카이제곱 분포를 사용.
- 프리드먼 검정이 보수적이므로(차이가 없다고 판별할 가능성이 큼) 개선된 프리드먼 검정을 사용.

	\tau_F = \frac{(N-1)\tau_{\chi^2}}{N(k-1)-\tau_{\chi^2}}\sim F_{k-1, (k-1)(N-1)}{% raw %}
	\tau_F = \frac{(N-1)\tau_{\chi^2}}{N(k-1)-\tau_{\chi^2}}\sim F_{k-1, (k-1)(N-1)}$$
	\tau_F = \frac{(N-1)\tau_{\chi^2}}{N(k-1)-\tau_{\chi^2}}\sim F_{k-1, (k-1)(N-1)}9636
	\tau_F = \frac{(N-1)\tau_{\chi^2}}{N(k-1)-\tau_{\chi^2}}\sim F_{k-1, (k-1)(N-1)}$$
	\tau_F = \frac{(N-1)\tau_{\chi^2}}{N(k-1)-\tau_{\chi^2}}\sim F_{k-1, (k-1)(N-1)}{% endraw %}
...
CD = q_\alpha \sqrt{\frac{k(k+1)}{6N}}{% raw %}
CD = q_\alpha \sqrt{\frac{k(k+1)}{6N}}$$
CD = q_\alpha \sqrt{\frac{k(k+1)}{6N}}9810
CD = q_\alpha \sqrt{\frac{k(k+1)}{6N}}$$
CD = q_\alpha \sqrt{\frac{k(k+1)}{6N}}{% endraw %}
...
- 알고리즘의 기대 예측값 (예측값의 평균 or 기댓값)
	...
	\bar f({\bf x}) = E_D[f({\bf x}, D)]{% raw %}
	\bar f({\bf x}) = E_D[f({\bf x}, D)]$$
	\bar f({\bf x}) = E_D[f({\bf x}, D)]10085
	\bar f({\bf x}) = E_D[f({\bf x}, D)]$$
	\bar f({\bf x}) = E_D[f({\bf x}, D)]{% endraw %}

- 분산

	var({\bf x}) = E_D[(f({\bf x}, D)-\bar f({\bf x}))^2]{% raw %}
	var({\bf x}) = E_D[(f({\bf x}, D)-\bar f({\bf x}))^2]$$
	var({\bf x}) = E_D[(f({\bf x}, D)-\bar f({\bf x}))^2]10138
	var({\bf x}) = E_D[(f({\bf x}, D)-\bar f({\bf x}))^2]$$
	var({\bf x}) = E_D[(f({\bf x}, D)-\bar f({\bf x}))^2]{% endraw %}

- 노이즈

	\epsilon^2 = E_D[(y_D-y)^2]{% raw %}
	\epsilon^2 = E_D[(y_D-y)^2]$$
	\epsilon^2 = E_D[(y_D-y)^2]10209
	\epsilon^2 = E_D[(y_D-y)^2]$$
	\epsilon^2 = E_D[(y_D-y)^2]{% endraw %}

- 편향(기대 결괏값과 실제 데이터의 차이)

	bias^2({\bf x}) = (\bar f({\bf x})-y)^2{% raw %}
	bias^2({\bf x}) = (\bar f({\bf x})-y)^2$$
	bias^2({\bf x}) = (\bar f({\bf x})-y)^210273
	bias^2({\bf x}) = (\bar f({\bf x})-y)^2$$
	bias^2({\bf x}) = (\bar f({\bf x})-y)^2{% endraw %}

- 일반 오차(Total error)가 편향, 분산, 노이즈의 합으로 분해될 수 있음.

	\begin{aligned} Error(f;D) =& E_D [(f({\bf x};D)-y_D)^2]\\ =& E_D[(f({\bf x};D)-\bar f({\bf x})+\bar f({\bf x})-y_D)^2]\\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y_D)^2]\\ &+ E_D[2(f({\bf x};D)-\bar f({\bf x}))(\bar f({\bf x})-y_D)] \\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y+y-y_D)^2] \\ =& var({\bf x})+((\bar f({\bf x})-y)^2+E_D[(y_D-y)^2]\end{aligned}{% raw %}
	...
	\begin{aligned} Error(f;D) =& E_D [(f({\bf x};D)-y_D)^2]\\ =& E_D[(f({\bf x};D)-\bar f({\bf x})+\bar f({\bf x})-y_D)^2]\\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y_D)^2]\\ &+ E_D[2(f({\bf x};D)-\bar f({\bf x}))(\bar f({\bf x})-y_D)] \\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y+y-y_D)^2] \\ =& var({\bf x})+((\bar f({\bf x})-y)^2+E_D[(y_D-y)^2]\end{aligned}10373
	\begin{aligned} Error(f;D) =& E_D [(f({\bf x};D)-y_D)^2]\\ =& E_D[(f({\bf x};D)-\bar f({\bf x})+\bar f({\bf x})-y_D)^2]\\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y_D)^2]\\ &+ E_D[2(f({\bf x};D)-\bar f({\bf x}))(\bar f({\bf x})-y_D)] \\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y+y-y_D)^2] \\ =& var({\bf x})+((\bar f({\bf x})-y)^2+E_D[(y_D-y)^2]\end{aligned}$$
	\begin{aligned} Error(f;D) =& E_D [(f({\bf x};D)-y_D)^2]\\ =& E_D[(f({\bf x};D)-\bar f({\bf x})+\bar f({\bf x})-y_D)^2]\\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y_D)^2]\\ &+ E_D[2(f({\bf x};D)-\bar f({\bf x}))(\bar f({\bf x})-y_D)] \\ =& var({\bf x}) + E_D[(\bar f({\bf x})-y+y-y_D)^2] \\ =& var({\bf x})+((\bar f({\bf x})-y)^2+E_D[(y_D-y)^2]\end{aligned}{% endraw %}


	\therefore Error(f;D) = bias^2({\bf x})+ var({\bf x})+\epsilon^2{% raw %}
	\therefore Error(f;D) = bias^2({\bf x})+ var({\bf x})+\epsilon^2$$
	\therefore Error(f;D) = bias^2({\bf x})+ var({\bf x})+\epsilon^210740
	\therefore Error(f;D) = bias^2({\bf x})+ var({\bf x})+\epsilon^2$$
	\therefore Error(f;D) = bias^2({\bf x})+ var({\bf x})+\epsilon^2{% endraw %}
