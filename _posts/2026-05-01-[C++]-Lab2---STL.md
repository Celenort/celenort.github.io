...
- `push_back(value)` : pushes the elements into a vector from the back
- `begin()`, `end()`
- `resize(n)` : resizes to given value, capacity remains
- `shrink_to_fit(n)` : make capacity to given value
...
- `insert(iterator, val)` : inserts given val into pos iter
- `erase(iterator)` : erases value at given pos iter
- `clear()` : clears the vec. size becomes 0
...
- `lower_bound(const g)` : first element ≥ g
- `upper_bound(const g)` : first element > g
...
- `insert(pair<int, int>(val,val));` : inserts given key. override not working
- `m[i]=j;` : Overriding works
- `count(key)` : if key exists return 1, else 0
- `lower_bound(g)`, `upper_bound(g)` , copy constructor all works same with set
