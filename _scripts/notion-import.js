...
// Load snippet from the same folder as this script: mathjax.html
...
function getMulti(props, names) {
  const p = getProp(props, names);
  return p?.multi_select ? p.multi_select.map((o) => o.name) : [];
}
...
// IMPORTANT FIX:
// basename만 쓰면 서로 다른 이미지가 같은 이름(image.png 등)을 가질 때
// 동일 이미지로 오인될 수 있으므로 hostname + pathname 전체를 key로 사용.
...
async function transformImagesInMarkdown(md, fileBase, prevImages) {
  ...
  for (const it of prevImages || []) {
    if (it && it.kind === "local" && it.norm) {
      prevByNorm.set(it.norm, it);
    }
  }
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const finalMarkdownPath = `${mediaSubpath}/${fileName}`;
    newManifest.push({
      kind: "local",
      norm,
      file: finalMarkdownPath,
    });

    replacements.push(`![${alt}](${finalMarkdownPath})`);
  }

  for (const it of prevImages || []) {
    if (it && it.kind === "local" && it.norm && !currentNorms.has(it.norm)) {
      const p = it.file ? it.file.replace(/^\//, "") : null;
...
(async () => {
  ...
  // 3) Determine changed pages
  const publishedPages = allPages.filter((p) =>
    getCheckbox(p.properties || {}, ["공개", "published", "Publish", "Visible", "Public"])
  );
  ...
  if (!forceFullOnce) {
    for (const p of publishedPages) {
      ...
      if (!prev || new Date(now) > new Date(prev)) {
        changed.push(p);
      }
    }
  }
  ...
  for (const r of changed) {
    ...
    if (ptitle?.length > 0) title = ptitle[0]?.plain_text || id;

    const safeTitle = String(title || id)
      .trim()
      .replace(/[\/\\:*?"<>|]/g, "")
      .replace(/\s+/g, "-");
    ...
    const descText = getPlainText(props, [
      "description",
      "Description",
      "설명",
      "요약",
    ...
    const propImageUrl = getImageUrl(props, [
      "image",
      "Image",
      "이미지",
      "대표이미지",
...
${mathjaxSnippet}`;
    ...
    state.pages[id] = {
      last_edited_time: iso(r.last_edited_time),
      outfile: filePath,
      images: sanitizedImages,
    };
  }

  // 5) Remove files for pages no longer in Notion
  if (allPages.length === 0) {
    console.log("Notion returned 0 pages; skip deletion to preserve existing _posts.");
  } else {
  ...
  // 6) Save checkpoint
  state.__full_rebuild_done = true;
